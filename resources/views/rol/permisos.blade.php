<x-layout bodyClass="g-sidenav-show bg-gray-200">
            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <form method="POST" action="{{ route('permisos.pantallas.sync') }}" class="me-3" id="syncPantallasForm">
                            @csrf
    ...
    <script>
            const setupPermisosPage = () => {
                const changes = new Set();
                const saveAllBtn = document.getElementById('saveAllChanges');
                const changesList = document.getElementById('changesList');
                const syncPantallasForm = document.getElementById('syncPantallasForm');
            ...
            // Sync screens form
            if (syncPantallasForm) {
                syncPantallasForm.addEventListener('submit', function() {
            ...
            async function showScreenDetails(screenId) {
                try {
                    showLoader('Cargando...');
                    const response = await fetch(`{{ url('/permisos-pantallas/pantalla') }}/${screenId}`);
            ...
            async function showRoleSummary(roleId) {
                try {
                    showLoader('Cargando...');
                    const response = await fetch(`{{ url('/permisos-pantallas/rol') }}/${roleId}/resumen`);
                    const data = await response.json();
            ...
            const handlePermisosLoad = () => {
                const root = document.getElementById('syncPantallasForm');
                if (!root) return;
                if (root.dataset.permisosInit === '1') return;
                root.dataset.permisosInit = '1';
                setupPermisosPage();
            };

            document.addEventListener('turbo:load', handlePermisosLoad);
            document.addEventListener('DOMContentLoaded', handlePermisosLoad);
    </script>

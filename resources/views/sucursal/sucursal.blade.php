          ...
          const setupSucursalPage = () => {
            ...
            async function fetchSucursales(page = 1, search = '', sortCol = '', sortDir = '') {
              try {
                ...
                const url = new URL(`{{ route('sucursal.fetch') }}`, window.location.origin);
            ...
            form.addEventListener('submit', async (e) => {
              ...
              const url = id ? `{{ url('sucursal') }}/${id}` : `{{ route('sucursal.store') }}`;
            ...
            window.__sucursalReload = () => {
              fetchSucursales(1, currentSearch, sortColumn, sortDirection);
            };
          };

          const handleSucursalLoad = () => {
            const root = document.getElementById('sucursales-table');
            if (!root) return;
            if (root.dataset.sucursalInit === '1') {
              if (typeof window.__sucursalReload === 'function') {
                window.__sucursalReload();
              }
              return;
            }
            root.dataset.sucursalInit = '1';
            setupSucursalPage();
          };

          document.addEventListener('turbo:load', handleSucursalLoad);
          document.addEventListener('DOMContentLoaded', handleSucursalLoad);
        </script>
      </div>
    </div>
    <style>
